...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        existing_s_nodes = self.s_levels[level]
        all_literals = [nodes.symbol for nodes in existing_s_nodes]

        for action in (self.problem.actions_list+self.noop_actions(all_literals)):
    ...
    def add_literal_level(self, level):
        ...
        previous_s_nodes = self.s_levels[level - 1]
        self.s_levels.append(set())
        for node in previous_s_nodes:
            self.s_levels[level].add(PgNode_s(node.symbol,node.is_pos))

        for a_node in action_nodes:
            effect_nodes = a_node.effnodes
            for s_node in effect_nodes:
                new_node = PgNode_s(s_node.symbol, s_node.is_pos)
                self.s_levels[level].add(new_node)
                a_node.children.add(new_node)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        a1 = node_a1.action
        a2 = node_a2.action
        for a1_effect_add in a1.effect_add:
            for a2_effect_rem in a2.effect_rem:
                if a1_effect_add == a2_effect_rem:
                    result = True
        for a2_effect_add in a2.effect_add:
            for a1_effect_rem in a1.effect_rem:
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        a1 = node_a1.action
        a2 = node_a2.action
        for a1_pos in a1.precond_pos:
            if a1_pos in a2.effect_rem:
                    result = True
        for a1_neg in a1.precond_neg:
            if a1_neg in a2.effect_add:
                    result = True
        for a2_pos in a1.precond_pos:
            if a2_pos in a1.effect_rem:
                    result = True
        for a2_neg in a1.precond_neg:
            if a2_neg in a1.effect_add:
                    result = True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for parent1 in node_a1.parents:
            for parent2 in node_a2.parents:
                if parent1.symbol==parent2.symbol and parent1.is_pos != parent2.is_pos or parent1.is_mutex(parent2):
                    result = True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        result = True
        for parent1 in parents_s1:
            for parent2 in parents_s2:
    ...
    def h_levelsum(self) -> int:
        ...
        for fluent in self.problem.goal:
            ...
            for level in range(len(self.s_levels)):
                if PgNode_s(fluent,True) in self.s_levels[level]:
                    h_levels.append(level)
                    break


        level_sum = sum(h_levels)
